...
| タブ | アイコン | 画面 |
|------|---------|------|
| タイムライン | update | タイムライン画面 |
| デスク | inbox | デスク画面（トリアージ） |
| 本棚 | library_books | ブックマーク一覧画面 |
...
**機能:**
- 通知データ（notifications テーブル）を小説情報付きで更新日時降順表示
- フィルターチップで種別絞り込み（すべて / 新話 / 新作 / 完結）
- 種別ごとの色分けアイコン（新話=青, 新作=緑, 完結=オレンジ）
- 未読は太字 + 未読バッジ（ドット）表示
- プルダウンリフレッシュ
- タップで小説URLを外部ブラウザで開く + 既読化
- コンパクトな ListTile ベースのカード表示
